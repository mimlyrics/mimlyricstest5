...
const VideoStream2 = () => {
  ...
  const PlayNextVideo = () => {
    let randomId = Math.floor(Math.random() * length)
    console.log(randomId);
    navigate(`/video/stream?videoId=${id}`)
      /*console.log(sVideo);
  ...
  " bg-sky-100 w-[90vw] md:w-[45%] h-[75vh] mx-2  rounded-lg "} src={path} controls />*/}
        ...
        {/** Control video */}
        <div className="cursor-pointer absolute top-[25vw]">
          <div>
            <FaBackward onClick={PlayRecentVideo} 
              className="absolute bg-gray-500 o left-[10vw] text-slate-300 w-6 h-6"/>
            <FaPause onClick={ToggleVideo} className="absolute left-[40vw] text-slate-300 w-6 h-6"/>
            <FaForward onClick={PlayNextVideo} className="absolute left-[80vw] text-slate-300 w-6 h-6"/>
          </div>
        </div>

        <div className="absolute top-3 ">
          <IoMdSettings onClick={() => setShowSettings(!showSettings)} className="w-7 h-7 opacity-50 cursor-pointer text-slate-400 hover:translate-y-[1px] hover:opacity-100 "/>
          {
            showSettings ? 
            <div className=" py-1 border mx-3 w-[95vw] rounded-lg border-opacity-40  border-cyan-900 opacity-100 absolute top-[85vw]">
              <div className=" ">
                <div className=" my-2 flex space-x-40">
                  <div className=" mx-3 flex space-x-4">
                    <IoMdVideocam className="w-6 h-6"/>
                    <h1>Quality</h1>
                  </div>
                  <div className=" flex space-x-2 ">
                    <h1>High Quality(360p)</h1>
                    <FaForward className="text-slate-500"/>
                  </div>
                </div>
                ...
                <div className=" flex space-x-40">
                  ...
                  <div className=" flex space-x-2 ">
                    <input type="radio" checked={isRepeat} onClick={() => setIsRepeat(!isRepeat)} />
                  </div>
                </div>
        ...
        <div className="absolute top-[37vh] md:top-[75vh] w-[98%] p-2 rounded-lg shadow-sm shadow-gray-100 bg-gray-600 bg-opacity-50">     
      ...
      <div className=" absolute top-[42vh] md:top-[81vh] left-[0vw] md:w-[55%] lg-w[45%] w-[85vw]">
        <div onClick={handleCurrentProgress} ref={progressContainerRef} className={` relative mx-7 w-[100%] lg:w-[85%] h-[10px] bg-slate-600 rounded-lg cursor-pointer `}>
      ...
      <div className=" grid grid-cols-2 lg:grid-cols-none md:absolute md:top-[90vh] left-0 md:-mt-5 mt-3">      
